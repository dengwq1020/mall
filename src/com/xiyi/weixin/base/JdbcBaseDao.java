...
public abstract class JdbcBaseDao extends NamedParameterJdbcDaoSupport
{
     ...
     * 
     */
    public List<Result> getResults(String sql){
    	return this.getJdbcTemplate().query(sql, new BeanPropertyRowMapper(Result.class));
    }
    /**
    ...
    {
        return this.getNamedParameterJdbcTemplate().queryForLong(sql, map);
    }
    /**
     * count,sum,max等查询
     *
     ...
     * @param sql  sql
     * @param cl   对象
     * 返回一个或多个Member,字段和字段类型必须和数据库的一样,否则会报错
     */
    public <T> List<T> getMember(String sql,Class cl){
    	return this.getJdbcTemplate().query(sql,new BeanPropertyRowMapper(cl));
    }
